...
var getRequestWithApi = function(apiKey) {
  return function(path, data, done) {
    ...
    request.get(url).then(globalResponseHandler({ url: url }, done));
  };
};
...
var postRequestWithApi = function(apiKey) {
  return function(path, data, done) {
    ...
    var requestData = {
      method: 'POST',
      data: querystring.stringify(_.extend({ 'key': apiKey }, data)),
      headers: {
        'X-HTTP-Method-Override': 'GET'
      }
    };
    request(url, requestData),then(globalResponseHandler(requestData, done));
  };
};
